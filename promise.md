...
运行getNumber并且在then中传了两个参数， then方法可以接受两个参数， **第一个对应resolve的回调， 第二个对应reject的回调**。 所以我们能够分别拿到他们传递过来的数据。 多次运行这段代码， 会随机得到两种结果：![resolve&rejecte](.\img\resolve&rejecte.png)
...
![race使用](.\img\race使用.png)

图片请求成功：

![race成功请求](.\img\race成功请求.png)
